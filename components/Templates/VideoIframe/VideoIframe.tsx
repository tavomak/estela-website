...
const VideoIframe: FC<Props> = ({ videoId, controls = false, muted = false }) => (
  <figure className='relative object-contain w-full h-full overflow-hidden aspect-video'>
      ...
      <div className={styles.videoForeground}>
        {isNumber(videoId) ? (
          ...
          <iframe
            ...
            allow='autoplay; fullscreen; picture-in-picture'
            allowFullScreen
            className='absolute pointer-events-none top-0 left-0 object-contain w-full h-full'
          />
        )}
      </div>
    </div>
